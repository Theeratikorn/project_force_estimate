...
class UR5eControlGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.input_amps = [QLineEdit("0.0025") for _ in range(5)]  # Create five amplitude input fields
        for i, input_amp in enumerate(self.input_amps):
            form_layout.addRow(f"Amplitude {i+1} (m):", input_amp)
    ...
    def start_robot(self):
        ...
        try:
            ...
            # Get user inputs
            try:
                amplitudes = [float(input_amp.text()) for input_amp in self.input_amps]
            ...
            for amp in amplitudes:
                ...
                self.export_csv(amp)  # Save data after each amplitude loop

            self.status_label.setText("Status: Completed")
            self.stop_robot()  # Stop the robot after finishing all loops
        except Exception as e:
